     ...
     이 부분 설정에 대한 어떠한 불이익은 전적으로 저희 책임이 아님을 다시 한번 말씀드립니다. <br/>
        ...
        <Accordion sx={{ width: '50%' }}>
          <AccordionSummary
            ...
            id="panel2-header"
          ><b>
             UTM 잘 안되는 경우 질문에 대한 답변 관련 양식</b>
          </AccordionSummary>
          <AccordionDetails>
          기존 UA의 경우 Override 기능이 있었기에 UTM을 사용하시더라도 UTM에 대한 소스/매체가 잡힐 수 있었습니다.<br />

다만, GA4의 경우 Override 기능이 없기에 UTM을 사용하시더라도 Google Ads 계정 설정에 자동 태그 추가 상태가 "예"로 되어 있을 경우, 
<br />GA4에서는 gclid가 우선시되어 UTM 값에 대한 소스/매체가 아닌 Google 광고로 인식하여 google/cpc 데이터가 집계될 수 있음을 안내 드립니다.


<br />
GA4에서는 수동 태그 우선 적용이 되지 않기에 데이터들이 혼합되어 집계될 수 있으며, 
<br />이와 같이 UTM이 보여지고 안보여지는 상황에 대한 추가 조사에 어려움이 있을 수 있는 점 안내드립니다.

<br />그렇기 때문에 GA4를 이용하시면서 UTM 데이터를 확인하실 수 있는 방법은 아래와 같습니다.

<br />Google Ads 광고 데이터를 UTM에 대한 소스/매체로 확인하셔야 한다면 Google Ads 계정 설정에서 
<br />자동 태그 추가 기능을 "아니오"로 변경해 주셔야 합니다.

<br />단, 자동 태그 추가 기능을 OFF 할 경우 시스템에서는 어떠한 광고를 통한 유입인지에 대한 것을 인식할 수 없게 되어 
<br />전환 수집에 이슈가 발생할 수 있기에 이 부분 참고해 주시기 바랍니다.<br />

더불어, 전환 수집에 이슈가 발생한다는 의미는 전환 데이터를 바탕으로 최적화될 수 있는 캠페인에 대한 전환 데이터가 
<br />원활하게 수집될 수 없어 최적화가 어렵기에 이러한 모든 부분을 고려하시어 자동 태그 추가 기능을 OFF 해주셔야 하는 점 안내드립니다.
<br />
결론적으로 기존 UA에서는 수동 태그 추가(UTM 값)를 자동 태그 추가(GCLID 값)보다 우선 적용이 가능했으나 
<br />GA4의 경우 해당 기능이 없기에 자동 태그 추가 기능을 활성화 하실 경우 UTM을 인식하지 못하여 
<br />google/cpc로 소스/매체 데이터가 잡히게되며, 자동 태그 추가 기능을 OFF 하시는 경우 UTM 값으로 잡힐 수 있겠으나 
<br />어떠한 광고를 통한 유입인지를 인식하기 어렵다보니 발생할 수 있는 이슈에 대해 안내드린 점 참고해 주시면 감사하겠습니다.
<br />
          </AccordionDetails>
        </Accordion>
